"""Deletion/insertion faithfulness for PubMed summarization models.

Adapted from billsum_faithfulness.py — uses gold-summary mean log-probability
as the scoring function and attention-based token importance over the article text.
"""
...
from .pubmed_signals import _PUBMED_SYSTEM_PROMPT, _build_pubmed_prompt, _extract_article_text
...
def _normalise_summary(text: str) -> str:
    ...
    stop_markers = ("Human:", "Assistant:", "User:", "###")
    cut_points = [text.find(m) for m in stop_markers if text.find(m) != -1]
    if cut_points:
        text = text[: min(cut_points)].strip()
    return re.sub(r"\s+", " ", text).strip()


def _build_pubmed_prompt_from_article_text(tokenizer, article_text: str) -> str:
    user_prompt = (
...
def _window_pubmed_row(
    *, row: Dict[str, Any], tokenizer, max_input_tokens: int,
) -> tuple[Dict[str, Any], Dict[str, Any]]:
    article_text = _extract_article_text(row["input"])
    full_prompt = _build_pubmed_prompt_from_article_text(tokenizer, article_text)
    empty_prompt = _build_pubmed_prompt_from_article_text(tokenizer, "")

    full_prompt_ids = tokenizer.encode(full_prompt, add_special_tokens=False)
    empty_prompt_ids = tokenizer.encode(empty_prompt, add_special_tokens=False)

    prefix_len = _common_prefix_len(full_prompt_ids, empty_prompt_ids)
    suffix_len = _common_suffix_len(full_prompt_ids, empty_prompt_ids, prefix_len=prefix_len)
    article_start = prefix_len
    article_end = len(full_prompt_ids) - suffix_len

    trunc_start = max(0, len(full_prompt_ids) - max_input_tokens)
    trunc_prompt_ids = full_prompt_ids[trunc_start:]

    visible_start = max(article_start, trunc_start) - trunc_start
    visible_end = max(min(article_end, len(full_prompt_ids)), trunc_start) - trunc_start
    visible_article_ids = trunc_prompt_ids[visible_start:visible_end]

    if not visible_article_ids:
        visible_article_ids = tokenizer.encode(article_text, add_special_tokens=False)[-max_input_tokens:]

    visible_article_text = tokenizer.decode(visible_article_ids, skip_special_tokens=True).strip()
    if not visible_article_text:
        visible_article_text = article_text[-256:]

    windowed = dict(row)
    windowed["input"] = _replace_article_text(row["input"], visible_article_text)

    info = {
        "original_article_token_count": len(tokenizer.encode(article_text, add_special_tokens=False)),
        "window_article_token_count": len(visible_article_ids),
...
def _article_token_importance(
    *, model, tokenizer, torch_mod, row: Dict[str, Any], max_input_tokens: int,
    ...
    article_ids = tokenizer.encode(_extract_article_text(row["input"]), add_special_tokens=False)
    span = _find_subsequence(full_ids, article_ids)
    if span is None:
    ...
    article_token_ids = full_ids[start:end]
    return article_token_ids, token_scores.tolist()
...
def _replace_article_text(raw_input: str, new_article_text: str) -> str:
    original = _extract_article_text(raw_input)
    if original in raw_input:
        return raw_input.replace(original, new_article_text, 1)
    if "### Article:" in raw_input:
        prefix, _, rest = raw_input.partition("### Article:")
        if "### Summary:" in rest:
...
def _perturb_article_tokens(
    *, row: Dict[str, Any], tokenizer, selected_positions: Sequence[int],
    step: int, steps: int, mode: str,
) -> Dict[str, Any]:
    article_token_ids = tokenizer.encode(_extract_article_text(row["input"]), add_special_tokens=False)
    cut = max(1, int(len(selected_positions) * (step / steps)))
    chosen = set(selected_positions[:cut])

    if mode == "deletion":
        kept_ids = [tok for idx, tok in enumerate(article_token_ids) if idx not in chosen]
    elif mode == "insertion":
        kept_ids = [tok for idx, tok in enumerate(article_token_ids) if idx in chosen]
    else:
        raise ValueError(f"Unsupported perturbation mode: {mode}")

    if not kept_ids:
        kept_ids = article_token_ids[:1]

    new_text = tokenizer.decode(kept_ids, skip_special_tokens=True).strip()
    if not new_text:
        new_text = _extract_article_text(row["input"])[:32]

    mutated = dict(row)
    mutated["input"] = _replace_article_text(row["input"], new_text)
    return mutated


def _faithfulness_for_row(
    *, model, tokenizer, torch_mod, row: Dict[str, Any],
    topk_ratio: float, steps: int, max_input_tokens: int,
) -> Dict[str, Any]:
    windowed_row, window_info = _window_pubmed_row(
        row=row,
        tokenizer=tokenizer,
    ...
    base_score, ref_token_count = _score_reference_logprob(
        ...
        row=windowed_row, max_input_tokens=max_input_tokens,
    )
    article_token_ids, token_scores = _article_token_importance(
        model=model, tokenizer=tokenizer, torch_mod=torch_mod,
        row=windowed_row, max_input_tokens=max_input_tokens,
    )
    ...
    for step in range(1, steps + 1):
        rec_del = _perturb_article_tokens(
            row=windowed_row, tokenizer=tokenizer, selected_positions=selected,
            step=step, steps=steps, mode="deletion",
        )
        del_score, _ = _score_reference_logprob(
            model=model, tokenizer=tokenizer, torch_mod=torch_mod,
            row=rec_del, max_input_tokens=max_input_tokens,
        )
        deletion_curve.append(float(del_score))

        rec_ins = _perturb_article_tokens(
            row=windowed_row, tokenizer=tokenizer, selected_positions=selected,
            step=step, steps=steps, mode="insertion",
        )
        ins_score, _ = _score_reference_logprob(
            model=model, tokenizer=tokenizer, torch_mod=torch_mod,
            row=rec_ins, max_input_tokens=max_input_tokens,
        )
    ...
    return {
        ...
        "article_token_count": len(article_token_ids),
        **window_info,
        "aopc": aopc,
...
def compute_pubmed_faithfulness(
    config_path: str,
    split: str = "test",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compute PubMed deletion/insertion faithfulness metrics.")
    ...
    out = compute_pubmed_faithfulness(
